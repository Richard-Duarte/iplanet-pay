...
export function GoogleAuthButton({
  ...
  async function signIn() {
    ...
    try {
      ...
      const next = nextPath && nextPath.startsWith("/") ? nextPath : "/app";
      ...
      if (oauthError) {
        setError(
          oauthError.message?.includes("provider is not enabled") ||
            ...
            ? "Login com Google ainda não está habilitado. Peça ao admin para ativar o provedor Google no Supabase Auth."
            : oauthError.message || "Não foi possível iniciar o login com Google.",
        );
        setLoading(false);
      }
  ...
  return (
    <div className="space-y-2">
      <Button
        ...
        onClick={() => void signIn()}
        leftIcon={<GoogleMark className="h-5 w-5" />}
      >
